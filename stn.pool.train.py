...
def get_vgg_train(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    ...
    # rpn_label = mx.symbol.Variable(name='rpn_label')
    # rpn_bbox_weight = mx.symbol.Variable(name='rpn_bbox_weight')
    # rpn_bbox_target = mx.symbol.Variable(name='rpn_bbox_target')
    ...
    rpn_cls_score = mx.symbol.Convolution(data=rpn_relu, kernel=(1, 1), pad=(0, 0), num_filter=2*num_anchors, name='rpn_cls_score')
    rpn_bbox_pred = mx.symbol.Convolution(data=rpn_relu, kernel=(1, 1), pad=(0, 0), num_filter=4*num_anchors, name='rpn_bbox_pred')
